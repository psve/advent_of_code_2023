...
func parseScratchCard(s string) ([]string, []string) {
	parts := strings.Split(strings.Split(s, ":")[1], "|")

	// Numbers are space aligned, which messes with splitting. Sanitize.
	wins := strings.Split(strings.TrimSpace(strings.ReplaceAll(parts[0], "  ", " ")), " ")
	ours := strings.Split(strings.TrimSpace(strings.ReplaceAll(parts[1], "  ", " ")), " ")

	return wins, ours
}

func getScratchCardMatches(wins, ours []string) int {
	// Store winning numbers in a set
	winners := make(map[string]any)
	for _, number := range wins {
		winners[number] = nil
	}

	// Check how many numbers we have
	matches := 0
...
func day04Part1() {
	...
	forEachLine(day04data, func(i int, line string) {
		wins, ours := parseScratchCard(line)
		matches := getScratchCardMatches(wins, ours)
		if matches != 0 {
			score += 1 << (matches - 1)
		}
	})
...
*/
func day04Part2() {
	cards := make([]int, strings.Count(day04data, "\n"))

	forEachLine(day04data, func(card int, line string) {
		wins, ours := parseScratchCard(line)
		matches := getScratchCardMatches(wins, ours)
		...
		// Add copies for each of the current card
		for i := 0; i < matches; i++ {
			cards[card+1+i] += cards[card]
		}
	})
