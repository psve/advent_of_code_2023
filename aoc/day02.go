...
*/
func day02Part1() {
	maxRed, maxGreen, maxBlue := int64(12), int64(13), int64(14)

	getID := func(line string) int64 {
		parts := strings.Split(line, ":")

		// Parse game ID
		id := must(stringToNumber(strings.Split(parts[0], " ")[1]))
		...
		for _, p := range strings.Split(parts[1], ";") {
			...
			for i := 0; i < len(tokens); i++ {
				number := must(stringToNumber(tokens[i]))
				i++
	...
	res := int64(0)
	forEachLine(day02data, func(i int, line string) {
		res += getID(line)
	})
...
*/
func day02Part2() {
	getPower := func(line string) int64 {
		red, green, blue := int64(0), int64(0), int64(0)

		// Parse draws
		for _, p := range strings.Split(strings.Split(line, ":")[1], ";") {
			tokens := strings.Split(strings.TrimSpace(p), " ")
