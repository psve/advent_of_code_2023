...
func parseHistories() ([][]int64, [][]int64) {
	histories := make([][]int64, 0)

	forEachLine(day09data, func(i int, line string) {
		values := strings.Split(line, " ")
		ints := make([]int64, len(values))
		for j, v := range values {
			ints[j] = must(stringToNumber(v))
		}
		histories = append(histories, ints)
	})

	scratchSpace := make([][]int64, len(histories[0]))
	for i := range scratchSpace {
		scratchSpace[i] = make([]int64, len(histories[0]))
	}

	return histories, scratchSpace
}

func predictValue(history []int64, scratchSpace [][]int64, back bool) int64 {
	// If predicting backwards, just flip the input and swap some signs below.
	if back {
		slices.Reverse(history)
	}

	scratchSpace[0] = history
	row := 0
	length := len(history)

	for i := 0; i < len(scratchSpace)-1; i++ {
		allZero := true

		for j := 0; j < length-row-1; j++ {
			scratchSpace[i+1][j] = scratchSpace[i][j+1] - scratchSpace[i][j]
			if back {
				scratchSpace[i+1][j] = -scratchSpace[i+1][j]
			}
			allZero = allZero && scratchSpace[i+1][j] == 0
		}
	...
	diff := scratchSpace[row][length-row-1]
	for i := row - 1; i >= 0; i-- {
		if back {
			diff = scratchSpace[i][length-i-1] - diff
		} else {
			diff = scratchSpace[i][length-i-1] + diff
		}
	}
...
The third history requires even more sequences, but its next value can be found the same
way:
...
*/
func day09Part1() {
	histories, scratchSpace := parseHistories()

	sum := int64(0)
	for _, history := range histories {
		sum += predictValue(history, scratchSpace, false)
	}
...
func day09Part2() {
	...
	sum := int64(0)
	for _, history := range histories {
		sum += predictValue(history, scratchSpace, true)
	}
