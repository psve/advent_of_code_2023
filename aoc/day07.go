...
var cardStrength = map[byte]int64{
	'2': 0,
	'3': 1,
	'4': 2,
	'5': 3,
...
var cardStrengthJoker = map[byte]int64{
	'J': 0,
	'2': 1,
	'3': 2,
	'4': 3,
...
func newHand(cards string, bid int64, joker bool) Hand {
	return Hand{
		cards:    cards,
		bid:      bid,
		handType: getType(cards, joker),
	}
}

func cmpHand(order map[byte]int64) func(a, b Hand) int {
	return func(a, b Hand) int {
		...
		for i := 0; i < 5; i++ {
			aStrength, bStrength := order[a.cards[i]], order[b.cards[i]]
...
func getType(cards string, joker bool) handType {
	...
	// It always makes sense to use the jokers as the card which has the most repetitions
	// already.
	first += jokers
...
func day07Part1() {
	...
	forEachLine(day07data, func(i int, line string) {
		parts := strings.Split(line, " ")
		hands = append(hands, newHand(
			parts[0],
			must(stringToNumber(parts[1])),
			false,
		))
	})

	slices.SortFunc(hands, cmpHand(cardStrength))

	res := int64(0)
	for i, hand := range hands {
		res += (int64(i) + 1) * hand.bid
	}
...
type; for example, QJJQ2 is now considered four of a kind. However, for the purpose of
breaking ties between two hands of the same type, J is always treated as J, not the card
...
func day07Part2() {
	...
	slices.SortFunc(hands, cmpHand(cardStrengthJoker))
