...
// This map is of the format <pipe><previous step> -> <next step>. For example, of you
// went south and landed on an "L" pipe, you next step is to the east.
var pipes = map[string]rune{
	"|N": 'N',
	"|S": 'S',
	"-E": 'E',
	"-W": 'W',
...
// This map gives the coordinate adjustment for each direction.
...
func parsePipeMap() ([][]string, int, int) {
	pipeMap := make([][]string, 0)
	startX, startY := 0, 0
	forEachLine(day10data, func(y int, line string) {
		if x := strings.IndexByte(line, 'S'); x != -1 {
			startX, startY = x, y
		}
		pipeMap = append(pipeMap, strings.Split(line, ""))
	})
	return pipeMap, startX, startY
}
...
*/
func day10Part1() {
	pipeMap, x, y := parsePipeMap()
	direction := '_'

	// Find connecting pipe
	for dir, xy := range directions {
		if y+xy[1] < 0 || y+xy[1] >= len(pipeMap) || x+xy[0] < 0 || x+xy[0] >= len(pipeMap[0]) {
			continue
		}
		pipe := pipeMap[y+xy[1]][x+xy[0]]
		if _, ok := pipes[pipe+string(dir)]; ok {
			direction, x, y = dir, x+xy[0], y+xy[1]
			break
		}
	}
	...
	// Traverse the pipes
	steps := 1
	for ; pipeMap[y][x] != "S"; steps++ {
		next, ok := pipes[pipeMap[y][x]+string(direction)]
		if !ok {
			panic(fmt.Sprintf("no pipe found for %s at %d,%d", pipeMap[y][x]+string(direction), x, y))
		}
		xy := directions[next]
		direction, x, y = next, x+xy[0], y+xy[1]
	}
