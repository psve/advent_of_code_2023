...
func parseSeedsAndMappings() ([]int64, [][]MappingInterval) {
	seeds := make([]int64, 0)
	mappings := make([][]MappingInterval, 0)

	forEachLine(day05data, func(i int, line string) {
		switch {
		case strings.HasPrefix(line, "seeds: "):
			parts := strings.Split(strings.TrimPrefix(line, "seeds: "), " ")
			for _, p := range parts {
				seeds = append(seeds, must(stringToNumber(p)))
			}
		case strings.HasSuffix(line, "map:"):
			mappings = append(mappings, make([]MappingInterval, 0))
		case line != "":
			parts := strings.Split(line, " ")
			dst := must(stringToNumber(parts[0]))
			src := must(stringToNumber(parts[1]))
			length := must(stringToNumber(parts[2]))
			mappings[len(mappings)-1] = append(mappings[len(mappings)-1], MappingInterval{
				start:  src,
				stop:   src + length,
				offset: dst - src,
			})
...
func mapSeed(seed int64, mappings [][]MappingInterval) int64 {
	for _, mapping := range mappings {
		for _, mi := range mapping {
			if seed >= mi.start && seed <= mi.stop {
				seed = seed + mi.offset
				break
			}
		}
...
*/
func day05Part1() {
	seeds, mappings := parseSeedsAndMappings()
	minimum := int64(math.MaxInt64)

	for _, seed := range seeds {
		mapSeed(seed, mappings)
		minimum = min(minimum, mapSeed(seed, mappings))
	}
