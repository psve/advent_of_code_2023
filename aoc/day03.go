...
//go:embed day03.data
var day03data string

func getPartLocations() (map[string]int, []string) {
	// Map 'x,y' to part number, basically a lazy 2D range tree
	digitLocations := make(map[string]int)
	partNumbers := make([]string, 0)

	// Get the location of all part numbers
	current := ""
	forEachLine(day03data, func(y int, line string) {
		for x, c := range line {
			if isDigit(c) {
				// Append another digit to current part number
				current += string(c)

				// Add coordinate to map
				digitLocations[fmt.Sprintf("%d,%d", x, y)] = len(partNumbers)
			} else if current != "" {
				// Looks like the current part number ended, start a new one
				partNumbers = append(partNumbers, current)
				current = ""
			}
		}

		// End of the line, make sure to save the current number, if any
		if current != "" {
			partNumbers = append(partNumbers, current)
			current = ""
		}
	...
	return digitLocations, partNumbers
}
...
*/
func day03Part1() {
	digitLocations, partNumbers := getPartLocations()

	// Check which symbols overlap with part numbers. One number might be tagged by
	// several symbols, so we maintain a bit map.
	tagged := make([]bool, len(partNumbers))
	forEachLine(day03data, func(y int, line string) {
		for x, c := range line {
			if c == '.' || isDigit(c) {
				continue
			...
			// No symbols at the edge, no need to do bounds correction
			for i := x - 1; i <= x+1; i++ {
				for j := y - 1; j <= y+1; j++ {
					if idx, ok := digitLocations[fmt.Sprintf("%d,%d", i, j)]; ok {
						tagged[idx] = true
					}
				}
			}
	...
	for i, v := range tagged {
		if v {
			sum += must(stringToNumber(partNumbers[i]))
		}
	}
...
func day03Part2() {
	...
	// Find all gears and calculate their ratio
	sum := int64(0)
	forEachLine(day03data, func(y int, line string) {
		for x, c := range line {
			if c != '*' {
			...
			// Keep track of part numbers next to '*'
			...
			ratio := int64(1)
			for i := range parts {
				ratio *= must(stringToNumber(partNumbers[i]))
			}
			sum += ratio
		}
	})
