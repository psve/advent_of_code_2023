...
func getSteps(start, instructions string, done func(string) bool, nodes map[string][2]string) (int64, string) {
	current := start
	steps := int64(0)
	length := int64(len(instructions))
	for ; ; steps++ {
		switch instructions[steps%length] {
		case 'L':
		...
		// We break out here to make sure we do at least one step.
		if done(current) {
			steps++
			break
		}
...
func parseNodes() (string, map[string][2]string, []string) {
	instructions := ""
	nodes := make(map[string][2]string)
	startNodes := make([]string, 0)

	forEachLine(day08data, func(i int, line string) {
		if line == "" {
			return
		}
		...
		start := a[0]
		left := strings.TrimPrefix(b[0], "(")
		right := strings.TrimSuffix(b[1], ")")
...
*/
func day08Part1() {
	instructions, nodes, _ := parseNodes()
	steps, _ := getSteps("AAA", instructions, func(n string) bool { return n == "ZZZ" }, nodes)
...
*/
func day08Part2() {
	instructions, nodes, current := parseNodes()

	// A few observations:
	//
	// - There are the same number of start and end points,
	// - Distinct starting points go to distinct end points,
	// - The length of the "tail" part of these walks seem to be the same as the length of
	//   the "cycle" part of these walks.
	//
	// This allows us to calculate the cycle length of each walk, and then calculate the
	// least common multiple, e.g. when they meet up.
	cycles := make([]int64, len(current))
	for i, n := range current {
		cycles[i], current[i] = getSteps(
			n,
			instructions,
			func(n string) bool { return strings.HasSuffix(n, "Z") },
			nodes,
		)
	}
